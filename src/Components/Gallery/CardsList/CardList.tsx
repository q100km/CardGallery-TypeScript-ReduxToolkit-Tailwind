...
import { selectFilterDiscriptionValue, selectFilterTitleValue, selectOnlyFavorite } from '../../../redux/slices/filterSlice'

const CardList = () => {
  const cardsArray = useSelector(selectCardsArray)
  const filterTitleValue = useSelector(selectFilterTitleValue)
  ...
  const filteredCards = cardsArray.filter((card: ICard): boolean => {
    const filterByTitle = card.title.toLowerCase().includes(filterTitleValue.toLowerCase())
